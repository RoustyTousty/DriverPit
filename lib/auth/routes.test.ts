...
import { sanitizeNextPath } from "./oauthCallback";
...
// otherwise a nudge from /online silently returns the player to /daily, and the
// symptom is one nobody would connect to an encoding rule.
...
describe("signInHref", () => {
  ...
  it("round-trips every destination the app actually links from", () => {
    for (const next of ["/daily", "/infinite", "/online", RESET_PASSWORD_PATH]) {
  ...
  it("encodes, so the destination cannot smuggle its own parameters", () => {
    ...
    const href = signInHref("/daily?foo=bar");
    expect(href).not.toContain("&");
    expect(href).toContain("%3F");

    const search = new URLSearchParams(href.slice(href.indexOf("?")));
    expect(sanitizeNextPath(search.get("next"))).toBe("/daily");
  });
});
