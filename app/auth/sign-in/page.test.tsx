...
describe("SignInPage", () => {
  ...
  it("shows a full account the way out instead of a form it doesn't need", () => {
    ...
    expect(screen.getByRole("link", { name: "Back to DriverPit" })).toHaveAttribute("href", "/daily");
  });
  ...
  it("shows neither while the identity is still resolving", () => {
    ...
    // the same class of mistake as /daily's replay flash.
  ...
  it("falls back to /daily rather than following a crafted next", () => {
    ...
    expect(screen.getByTestId("auth-panel")).toHaveAttribute("data-next", "/daily");
  });
});
