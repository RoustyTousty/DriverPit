...
import { DEFAULT_NEXT, sanitizeNextPath } from "@/lib/auth/oauthCallback";
...
export default function SignInPage() {
  ...
  //
  // Through sanitizeNextPath, so a crafted value degrades to DEFAULT_NEXT
  // rather than becoming an open redirect: this ends up inside the `redirectTo`
  // handed to Supabase. The initial state is that same constant rather than a
  // second literal, so "where does a player with no ?next= end up" has one
  // answer.
  ...
  // separately and identity lands first (that split is why the daily board
  ...
  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center gap-6 px-4 py-16">
      ...
      >
        {/* A step up from the top bar's h-12, not a hero: this page has no other
            chrome, so the logo says where you are -- but it sits above a form,
            and at h-18+ it stopped being a header and started being the
            content. */}
        <Image src={driverpitBanner} alt="DriverPit" priority className="h-14 w-auto sm:h-16" />
      </Link>
