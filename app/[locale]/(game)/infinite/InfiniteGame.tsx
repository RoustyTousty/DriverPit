...
export function InfiniteGame({ allDrivers }: { allDrivers: DriverWithActivity[] }) {
  ...
  const { identityStatus, ensureIdentity } = useAuthIdentity();
  ...
  // The in-flight (or settled) promise for the CURRENT round's start. Doubles
  // as the "has a round ever been started" latch for the deferred path, and as
  // the thing a guess awaits when someone types faster than the round starts.
  const roundRef = useRef<Promise<void> | null>(null);
  ...
  function beginRound(next: DriverFilter, existing?: Promise<void>) {
    ...
    const started = (async () => {
      try {
        // infinite_start_round authorizes through auth.uid(), so a visitor who
        // has not been signed in yet needs an identity before the round exists
        // (roadmap Pass 4a). On every other path this resolves off a local
        // getSession with no network hop.
        if (!(await ensureIdentity())) throw new Error("no identity");
    ...
    roundRef.current = started;
    startTransition(async () => {
      await started;
    });
  }

  // Mount — but NOT for a visitor who has never been signed in. Starting a
  // round is a write that needs an identity, so doing it on mount would mint an
  // auth.users row for every crawl of /infinite, which is the cost roadmap Pass
  // 4a removes. Such a visitor gets an empty, playable-looking board and the
  // round starts the moment they touch it (onFocusCapture below) -- which for a
  // human is always before they can guess, and for a crawler is never.
  //
  // Anyone with a session keeps the old behaviour exactly: a driver is waiting
  // by the time the page paints.
  useEffect(() => {
    if (identityStatus === "loading") return;
    if (identityStatus === "anonymous") {
      // Not "loading": the overlay would otherwise sit over the board forever.
      setStatus("playing");
      return;
    }
    if (roundRef.current || startingRef.current) return;
    beginRound(filter, consumeInfiniteRoundPrefetch(filter) ?? undefined);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [identityStatus]);

  // Starts the deferred round on the first interaction. Latched by roundRef, so
  // returning focus to the input between guesses does nothing.
  ...
  function handleSelect(driver: DriverOption) {
    ...
    // The race guard for the deferred path: focus starts the round, but a fast
    // typist can pick a driver before it lands. Starting one here if focus
    // somehow never fired (a programmatic selection) keeps the guess from being
    // submitted against a round that does not exist.
    if (!roundRef.current) beginRound(filter);
    ...
    startTransition(async () => {
      try {
        // Resolved already on every ordinary guess; only the very first one on
        // the deferred path ever waits here.
        if (round) await round;
    ...
    <div className="relative mx-auto flex w-full flex-col gap-4 px-4 py-6" aria-busy={isLoading}>
      <header className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-muted">{modes("infinite")}</p>
        </div>
      ...
      {status === "error" ? (
        ...
        <>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {/* Starts the deferred round (and mints the identity it needs) the
                first time a person touches the input. See the mount effect. */}
            <div onFocusCapture={handleFirstInteraction}>
              <DriverAutocomplete
                drivers={poolDrivers}
