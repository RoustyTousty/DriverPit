...
- **On read** — the half that's easy to miss: nothing writes `user_stats` on a day you skip, so a stored streak stays **frozen at its last value forever** unless every reader re-evaluates it. A stored streak counts as 0 unless `last_daily_date` is today or yesterday (today's puzzle isn't missed until the UTC day ends). Applied in `AuthProvider.toUserStats` for the viewer's own stats, and **in SQL in the `leaderboard` view** for everyone else. The decay has to live in the view rather than in the client because any ordering on it happens inside the database, where a client-side adjustment arrives too late to affect who got the slot — that was literal when the streak board ranked `current_streak`; since drizzle/0060 it ranks the lifetime `max_streak` and nothing orders by the live streak at all. The view keeps the decay anyway: `current_streak` is still a public column on a public view, and a number that is wrong for an abandoned account is wrong whether or not something currently sorts by it.
...
The top-bar **cup** button (left of the logo) opens the **global Leaderboard** — not personal stats, which live in Settings → Statistics. Three boards, tabbed: **best daily streak**, **total daily wins** and **duel rating**. Full accounts only are ranked (`leaderboard` view filters `is_guest = false`); guests see the board with a "Save your progress" upgrade prompt. A viewer outside the rendered slots gets their own real rank appended (`myStreakRank` / `myWinsRank` / `myDuelRank`), counted against everyone rather than within the fetched page — one query, all three ranks, as correlated counts beside the viewer's own row.

**The streak board ranks `max_streak`, the lifetime best — not the live streak** (drizzle/0060). A live streak is the number a player watches; a *board* of live streaks is one nearly everybody sits at 0 or 1 on, and it resets to a wall of ties every time a day is missed. The lifetime best is legitimately immortal, which is why it needs none of the 0037 decay: an account that stops playing keeps its record, and that is the claim the board is making. The two columns sit side by side on the same view, so ordering and ranking on different ones is a live hazard — `rank.test.ts` asserts each expression does *not* mention its sibling.

**The wins board is `user_stats.wins`, exposed on the view as `daily_wins`** — the rename is the point, since `duel_wins` was already there and a rank subquery reading the wrong one parses, runs and returns a plausible number. Both of the new boards are small integers, so ties are the common case: every board's `ORDER BY` carries `id` as a tie-break, or the same board comes back in a different order on the next open, rows swapping places for no reason the player did.
...
**That redirect lives in `next.config.ts`, and it had to move there because a page could not do it** (2026-08-17). Pass 5 implemented it as `app/[locale]/(game)/daily/page.tsx` calling `permanentRedirect()` — the documented way, and wrong *here*: that route sits under the `(game)` layout, which renders the top bar, the ad slot, four marketing sections and an async `NewsSection`. React streams that shell as it renders, so by the time the page component threw its redirect the response had already begun, and **a status code cannot be changed after the first byte**. Next's fallback is a `<meta http-equiv="refresh">` in the body and a **200 OK**. Measured against production: `/daily` returned 200 with 128 KB of game shell, no canonical (the page never reached its own metadata, so it inherited the root layout's) and `robots: index, follow` — a full, indexable duplicate of `/`. Search Console reported it as *"Duplicate without user-selected canonical"*, which was an accurate description. A config redirect cannot have this bug because nothing renders: it is step 2 of Next's routing order (headers → redirects → middleware → filesystem), so it answers before next-intl's rewrite and before React is involved. **One entry per locale**, not a `/:locale/daily` pattern — that also matches `/anything/daily` and would 308 it to a 404 — and the locale is carried through, since `/es/daily` landing on the English home page un-translates the site for exactly the inbound traffic the redirect exists to keep. The general rule: **`redirect()` in a page under a layout that streams is not a redirect**; put a whole-route redirect in the config, where it is free and cannot be downgraded.

**`npm run seo:audit` now checks retired URLs, in every locale** (`checkLegacyRedirect`). It did not, despite `docs/seo-roadmap.md`'s Pass 5 acceptance claiming it "proves `/daily` 308s" — the sitemap check catches a *listed* URL that redirects, and there was nothing checking a *delisted* URL that fails to. A **200 is reported as its own failure** rather than as an unexpected status, because that is the shape this takes: a 404 would have been noticed in a day, and a 200 looked entirely healthy for nine.
...
**Routes that are not documents say so with `X-Robots-Tag: noindex`, set in `next.config.ts`'s `headers()`.** Two of them: `/manifest.webmanifest` and the generated OG card (`/opengraph-image`, one entry per locale). Google discovers both from the page — `<link rel="manifest">` and `og:image` — crawls them, and files them under *"Crawled — currently not indexed"*, which is the bucket meaning "we judged this not worth indexing". A header cannot be a `<meta>` here (neither route emits HTML), and being explicit keeps that bucket meaning what it should: pages with a quality problem, not file formats that were never candidates. Safe on both — browsers ignore the header when installing a PWA, and the social scrapers that matter for a card (Facebook, X, LinkedIn, Slack, Discord) do not consult robots directives at all, they fetch the URL the tag names. The one thing given up is the card appearing in Google Images, which is not somewhere a generic branded card earns anything. **A new non-HTML route that something links to belongs in that list**; `/api/*` does not, being disallowed in `robots.ts` already.
...
view (drizzle/0009, updated in 0037 and 0060), which exposes public columns only and is checked against RLS
...
- **Streaks are decayed on every read**, never trusted from the stored column — in `AuthProvider` for the viewer and in SQL in the `leaderboard` view for everyone else. A stored `current_streak` with a stale `last_daily_date` is meaningless by design. (`max_streak`, which the streak board now ranks, is a lifetime record and never decays.)
