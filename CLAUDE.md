...
**Amended 2026-08-15, with the risk accepted explicitly: the word is now permitted in exactly one shape, and `faq.items.wordleLike` is the only place it appears.** The blanket ban cost the site the one query it cannot otherwise reach — people type "F1 Wordle" for this genre and nothing here matched it. What was reinstated is *nominative* use: naming another product to describe how this one compares, inside an answer that states the differences and disclaims affiliation outright ("DriverPit is an independent fan project and is not affiliated with Wordle or The New York Times"). That is a materially different act from the one the takedowns above were aimed at, which is using the mark as a **source identifier for your own product** — an `<h1>` or a `<title>` reading "F1 Wordle".

So the rule is now about shape, not count. The word may appear **where the site is being compared to Wordle and says it is not Wordle**. It must not appear in a page title, an `og:title`, a heading, a meta description, or the GitHub repository description (set on GitHub, not in this repo) — in any of those it reads as branding, which is the form with real exposure. The entry sits first in `FAQ_KEYS` so it is also emitted in the `/faq` page's `FAQPage` JSON-LD; the home page's `FaqTeaser` keeps its own keys and is the next placement to use if one is ever wanted, in the same comparative form. A grep for `Wordle` across `messages/`, `components/`, `app/` and `lib/` should return that one entry's `q` and `a` per locale and the explanatory comment in `lib/marketing/faqContent.ts`, and nothing else.
